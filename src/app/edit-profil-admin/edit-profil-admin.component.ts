...
export class EditProfilAdminComponent implements OnInit{
...
if (!this.adminData.nom || !this.adminData.prenom || !this.adminData.dateNaissance || !this.adminData.email || !this.adminData.numTelephone || !this.adminData.adresse ) {
  alert('Veuillez remplir tous les champs.');
  return; // Ne soumettez pas le formulaire si un champ est vide
}

   
    this.personnelService.updateAdmin(this.adminData).subscribe(
      (admin) => {
...
validateEmail(): string {
  ...
  const emailString = String(this.adminData.email);
...
getDateOfBirthError(): number {
  ...
  if (!this.adminData.dateNaissance) {
    return 1; // date de naissance requise
  }
  // Vérifiez si la date de naissance est une date valide
  const dateNaissance = new Date(this.adminData.dateNaissance);
    ...
    this.personnelService.getAdminData().subscribe(
      (data) => {
        ...
        if (this.adminData?.dateNaissance) {
          ...
          this.adminData.dateNaissance = new Date(this.adminData.dateNaissance).toISOString().split('T')[0];
        }
      },
      (error) => {
